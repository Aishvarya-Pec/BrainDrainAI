...
import re
import random
from utils.fireworks_helper import generate_text

QUIZ_MODEL = "accounts/fireworks/models/kimi-k2p6"
...
def _coerce_answer_index(ans, options, explanation: str | None = None):
    ...
    if not s and explanation:
        s = str(explanation)
    # Try number in text (e.g., "2", "Option 3")
...
def _sanitize_quiz_dict(data: dict) -> dict:
    items = data.get("quiz") or data.get("questions") or data.get("items") or []
    out = []
    for q in items:
        ...
        out.append({"question": question, "options": options, "answer_index": idx, "explanation": explanation})
    return {"quiz": out}


def _difficulty_examples(difficulty: str) -> str:
    examples = {
        "easy": "Easy: Recall/recognition questions. Single-concept definitions and direct facts. Short stems, no multi-step reasoning. Distractors clearly incorrect; avoid tricks.",
        "medium": "Medium: Application/comprehension. Small scenarios with one-step reasoning or calculation. Plausible distractors reflecting common mistakes. Moderate stem length.",
        "hard": "Hard: Multi-step reasoning/calculation, integrate multiple concepts. Realistic scenarios or code snippets requiring analysis. Distractors include subtle misconceptions; careful reading required.",
        "god level": "God Level: Graduate/olympiad-style difficulty. Novel edge-case scenarios, multi-part reasoning, proof sketches or counterexamples. Numeric problems needing multiple computations; code with tricky edge cases. Distractors are very plausible but wrong for subtle reasons. Explanations briefly justify the correct choice and why others fail.",
    }
    return examples.get(difficulty, "")


def _synthesize_fallback(base: str, needed: int) -> list:
    templates = [
    ...
    while len(gen) < needed:
        ...
        gen.append({
            "question": stem, "options": options, "answer_index": (i % 4),
            "explanation": "The correct option matches the stem; others are distractors.",
        })
        i += 1
    return gen
...
def create_quiz_json(topic_or_text: str, difficulty: str = "easy", num_questions: int = 10) -> dict:
    ...
    schema = {"quiz": [{"question": "", "options": ["", "", "", ""], "answer_index": 0, "explanation": ""}]}
...
"""
    ...
    try:
        raw = generate_text(QUIZ_MODEL, prompt.strip(), temperature=temperature, max_tokens=max_toks)
    except Exception:
        return {"quiz": []}
...
"""
        try:
            raw2 = generate_text("accounts/fireworks/models/deepseek-v4-pro", strict_prompt.strip(), temperature=0.2, max_tokens=max_toks)
            try:
            ...
            for i in range(num_questions):
                ...
                gen.append({"question": stem, "options": opts, "answer_index": (i % 4), "explanation": "The correct option matches the stem; others are distractors."})
    ...
    for q in quiz:
        ...
        cleaned.append({"question": stem, "options": opts, "answer_index": ans, "explanation": exp})
        seen.add(key)

    if len(cleaned) < num_questions:
        fill = _synthesize_fallback(topic_or_text, num_questions - len(cleaned))
        for q in fill:
    ...
    final = [_shuffle_question(q) for q in cleaned[:num_questions]]
    return {"quiz": final}